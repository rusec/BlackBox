...
tsc  
try {
    pkg --out-path releases/latest/x64 -t 'node16-linux-x64,node16-win-x64,node16-macos-x64' -c ./package.json ./dist/Tortilla.js
    Write-Output 'Finished x64'
    pkg --out-path releases/latest/latest -t 'node16-linux,node16-win,node16-macos' -c ./package.json ./dist/Tortilla.js
    ...
    Write-Output 'UNABLE TO CREATE ARM Binaries PLEASE RUN pkg --out-path releases/latest/arm -t "node16-linux-arm64,node16-macos-arm64" ./dist/Tortilla.js'
}
catch {
